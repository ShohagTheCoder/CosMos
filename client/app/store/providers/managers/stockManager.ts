...
export class StockManager<T extends StockState> extends StateManager<T> {
    constructor(
        initialState: T,
        ...
        // Update total price of stock if products change
        ...
        // Update due price of stock if totalPrice change
    ...
    removeTo(id: string | undefined = undefined): this {
        ...
        if (currentIndex === -1) {
            console.log("Product ID not found in the stock");
            return this;
        }
    ...
    changeActiveProduct(val: number) {
        const stockProductsKey = Object.keys(this.get("products"));
        const activeProduct = this.get("activeProduct");

        if (!activeProduct || stockProductsKey.length <= 1) return this;

        const currentIndex = stockProductsKey.indexOf(activeProduct);
        const newIndex =
            (currentIndex + val + stockProductsKey.length) %
            stockProductsKey.length;

        this.set("activeProduct", stockProductsKey[newIndex]);
